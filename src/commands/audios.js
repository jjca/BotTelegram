...
app.bot.on("audio", function(msg) {
    ...
    if (msg.text.toString().toLowerCase().includes(pmmgva)) {
        app.bot.sendVoice(chatId, "../files/audios/mamagueva.mp3");
    }

    const maricodv = "marico de verdad";
    if (msg.text.toString().toLowerCase().includes(maricodv)) {
        app.bot.sendVoice(chatId, "../files/audios/marico.mp3");
    }

    const volo = "voló";
    if (msg.text.toString().toLowerCase().includes(volo)) {
        app.bot.sendVoice(chatId, "../files/audios/volo.mp3");
    }
